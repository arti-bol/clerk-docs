...
When backwards incompatible changes are necessary for an API, new versions must be released to prevent disruption to existing applications. These versions are identified by their release date, such as 2024-01-02. Both the frontend and backend APIs follow the same version.
